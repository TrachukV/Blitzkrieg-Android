...
void TestDragKeepsTheButtonDown()
{
    printf( "a drag holds the button: that is the selection box\n" );
    CRecogniser r;
    r.SetDisplayDensity( DPI );
    SLog log;
    ...
    Check( log.Count( ACTION_LEFT_UP ) == 0, "button stays down through the drag" );
    Check( log.Count( ACTION_MOUSE_MOVE ) > 0, "the cursor followed" );
    Check( log.Count( ACTION_RIGHT_CLICK ) == 0,
           "a moving finger is never an order, however long it takes" );

    Feed( &r, &log, PHASE_UP, &f, 1, 1400 );
    Check( log.Count( ACTION_LEFT_UP ) == 1, "released at the end" );
}
...
void TestHoldBecomesAnOrder()
{
    ...
    Check( log.Count( ACTION_LEFT_UP ) == 1,
           "the left button was let go first, so nothing gets selected" );
...
void TestSecondFingerCancelsTheClick()
{
    ...
    Check( log.Count( ACTION_LEFT_UP ) == 1, "the press was undone" );
    Check( r.IsTwoFinger(), "now a camera gesture" );
    ...
    Check( log.Count( ACTION_LEFT_DOWN ) == 1, "and no second press appeared" );
}

void TestTwoFingerDragScrolls()
{
    printf( "two fingers moving together scroll the camera\n" );
    CRecogniser r;
    r.SetDisplayDensity( DPI );
    SLog log;
    ...
    // Both fingers travel right and down, keeping their distance, so this is
    // a scroll and not a zoom.
    for ( int i = 1; i <= 8; ++i )
    {
    ...
    Check( log.Count( ACTION_SCROLL_LEFT ) > 0,
           "fingers to the right bring the land on the left into view" );
    Check( log.Count( ACTION_SCROLL_UP ) > 0, "and downward brings the land above" );
    Check( log.Count( ACTION_WHEEL ) == 0, "a parallel drag is not a zoom" );

    Feed( &r, &log, PHASE_SECOND_UP, fingers, 1, 1300 );
    // Every key that went down has come back up.
    Check( log.Count( ACTION_SCROLL_LEFT ) % 2 == 0, "the left key was released" );
    Check( log.Count( ACTION_SCROLL_UP ) % 2 == 0, "the up key was released" );
}
