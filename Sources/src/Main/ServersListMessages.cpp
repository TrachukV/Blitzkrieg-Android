...
void CServerInfoRefreshed::SendToUI()
{
	...
	(
		SToUICommand(
			...
			new SUIServerInfo(
					wUniqueServerID, Bk1AsUtf16( szGameName.c_str() ), szMapName.c_str(), nCurPlayers, nMaxPlayers, bPasswordRequired,
...
void CServerRemoved::SendToUI()
{
	...
	(
		SToUICommand(
			EMTUC_DELETE_SERVER,
			new SUIServerInfo( wUniqueServerID, Bk1AsUtf16( L"" ), "", -1, -1, false, false, -1, "", "", true, CMapInfo::TYPE_NONE, SMultiplayerGameSettings() )
		)
	);
