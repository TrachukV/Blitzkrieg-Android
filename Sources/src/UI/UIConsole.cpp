...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static const int OPEN_TIME = 200;						//¬рем€ открыти€ и закрыти€ консольки в миллисекундах
static const int CONSOLE_HEIGHT = 240;			//¬ысота консоли в пикселах
static const int TEXT_LEFT_SPACE = 20;			//ќтступ от левого кра€ экрана до текста в консоли
static const int TEXT_VERTICAL_SIZE = 20;		//–азмер шрифта по вертикали
static const int MINUS_PAGE_SIZE = 5;				//—пециальна€ константа отступа дл€ PgUp PgDown,
static const int CURSOR_ANIMATION_TIME = 400;		//период переключени€ курсора
...
bool CUIConsole::IsVisible()
{
	if ( bAnimation && dwLastCloseTime > 0 )
		return false;					//консолька закрываетс€
...
void CUIConsole::ShowWindow( int _nCmdShow )
{
	if ( _nCmdShow != UI_SW_HIDE && _nCmdShow != UI_SW_MINIMIZE )
	{
		...
		//посылаем наверх сообщение чтобы переключилс€ TEXT_MODE
		SUIMessage msg;
	...
	else
	{
		...
		//посылаем наверх сообщение чтобы вырубилс€ TEXT_MODE
		SUIMessage msg;
...
void CUIConsole::Reposition( const CTRect<float> &rcParent )
{
	...
	//считаю, что консол€ €вл€етс€ child дл€ screen
	rc.x1 = rcParent.x1;
	rc.x2 = rcParent.x2;
	SetScreenRect( rc );
	UpdateSubRects();
...
bool CUIConsole::Update( const NTimer::STime &currTime )
{
	...
	{
		//консоль в процессе открыти€
		//обновл€ем координаты консоли
	...
	{
		//≈сли консоль закончила свое открытие
		rc.top = 0;
	...
	{
		//≈сли консоль закончила свое закрытие
...
bool CUIConsole::OnChar( int nAsciiCode, int nVirtualKey, bool bPressed, DWORD keyState )
{
	...
	//≈сли печатный символ, то просто выводим его
//	if ( isprint( nAsciiCode ) )
	...
	//≈сли не печатный символ, то обрабатываем дополнительное управление
	switch( nVirtualKey )
	{
	case VK_RETURN:
		nBeginCommand = -1;
	...
	case VK_LEFT:
		...
		{
			//≈сли нажата crtl и стрелка влево, то сдвигаемс€ влево на одно слово
	...
	case VK_RIGHT:
		...
		{
			//≈сли нажата crtl и стрелка вправо, то сдвигаемс€ вправо на одно слово
			if ( nCursorPos < szEditString.size() )
			{
...
// CRAP}
	...
	//проверим, вдруг эта команда зарегистрирована дл€ выполнени€ в скрипте консоли, тогда выполним ее
