...
inline const WORD* GetGlobalWVar( const char *pszValueName, const WORD *defval = (const WORD *)L"" ) { const WORD *pszVal = GetSingleton<IGlobalVars>()->GetWVar( pszValueName ); return pszVal == 0 ? defval : pszVal; }
